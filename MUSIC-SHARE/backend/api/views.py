...
from .recommender_system import generate_recommendation
...
class RecommendedSongs(APIView):
    def get(self, request, title):
        songs = generate_recommendation(song_title=title)
        songs_dict = {
            "songs": songs
        }
        return Response(songs_dict)
...
class UserDelPlaylists(APIView):
    ...
    def get(self, request, playlist_id, username):
        songs = Contains.objects.filter(
            playlist_id=playlist_id).filter(username=username)
...
class UserUnlikeMusic(APIView):
    def delete(self, request, username, song_id):
        try:
            likedmusic = Likes.objects.filter(
...
class ArtistSongs(APIView):
    def get(self, request, artist_id):
        all_songs = Releases.objects.filter(artist_id=artist_id)
